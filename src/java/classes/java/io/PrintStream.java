 ...
 * streams, a <code>PrintStream</code> never throws an <code>IOException</code>;
 * instead, exceptional situations merely set an internal flag that can be
 * tested via the <code>checkError</code> method. Optionally, a
 * <code>PrintStream</code> can be created so as to flush automatically; this
 * means that the <code>flush</code> method is automatically invoked after a
 * byte array is written, one of the <code>println</code> methods is invoked, or
 * a newline character or byte (<code>'\n'</code>) is written.
 *
 * <p>
 * All characters printed by a <code>PrintStream</code> are converted into bytes
 * using the platform's default character encoding. The <code>{@link
 * PrintWriter}</code> class should be used in situations that require writing
 * characters rather than bytes.
 *
 * <p>
 * Compared to JDK, this implementation is NOT synchronized. In addition, it
 * provides protected <code>write()</code> methods that make extending the class
 * easier and subclasses slimmer.
 *
 * @author Frank Yellin
 * @author Mark Reinhold
 * @author Massimiliano "Maxi" Zattera (adaptations for b2fJ)
 * @since JDK1.0
 */
...
//	private Formatter formatter;
	 ...
	 * dependency on java.util.Objects.requireNonNull. PrintStream is loaded early
	 ...
	 * 
	 *                                      private static Charset toCharset(String
	 *                                      csn) throws UnsupportedEncodingException
	 *                                      { requireNonNull(csn, "charsetName");
	 *                                      try { return Charset.forName(csn); }
	 *                                      catch (IllegalCharsetNameException |
	 *                                      UnsupportedCharsetException unused) { //
	 *                                      UnsupportedEncodingException should be
	 *                                      thrown throw new
	 *                                      UnsupportedEncodingException(csn); } }
	 */
	 ...
	 * super(out); this.autoFlush = autoFlush; }
	 */

	/*
	 * Variant of the private constructor so that the given charset name can be
	 * verified before evaluating the OutputStream argument. Used by constructors
	 * creating a FileOutputStream that also take a charset name.
	 * 
	 * private PrintStream(boolean autoFlush, Charset charset, OutputStream out)
	 * throws UnsupportedEncodingException { this(autoFlush, out, charset); }
	 */
	 ...
	 * @since 1.4
	 * 
	 *        public PrintStream(OutputStream out, boolean autoFlush, String
	 *        encoding) throws UnsupportedEncodingException { this(autoFlush,
	 *        requireNonNull(out, "Null output stream"), toCharset(encoding)); }
	 */
	 ...
	 *                               checkWrite(fileName)} denies write access to
	 *                               the file
	 *
	 * @since 1.5
	 * 
	 ...
	 * necessary intermediate {@link java.io.OutputStreamWriter OutputStreamWriter},
	 ...
	 *                 charset}
	 *
	 * @throws FileNotFoundException        If the given file object does not denote
	 *                                      an existing, writable regular file and a
	 ...
	 * @since 1.5
	 * 
	 *        public PrintStream(String fileName, String csn) throws
	 *        FileNotFoundException, UnsupportedEncodingException { // ensure
	 *        charset is checked before the file is opened this(false,
	 *        toCharset(csn), new FileOutputStream(fileName)); }
	 */
	 ...
	 *             file exists, then it will be truncated to zero size; otherwise, a
	 *             new file will be created. The output will be written to the file
	 *             and is buffered.
	 *
	 * @throws FileNotFoundException If the given file object does not denote an
	 *                               existing, writable regular file and a new
	 ...
	 * @throws SecurityException            If a security manager is present and
	 *                                      {@link SecurityManager#checkWrite
	 *                                      checkWrite(file.getPath())} denies write
	 *                                      access to the file
	 *
	 * @throws UnsupportedEncodingException If the named charset is not supported
	 *
	 * @since 1.5
	 * 
	 *        public PrintStream(File file, String csn) throws
	 *        FileNotFoundException, UnsupportedEncodingException { // ensure
	 *        charset is checked before the file is opened this(false,
	 *        toCharset(csn), new FileOutputStream(file)); }
	 */
	...
	/**
	 * Flushes the stream. This is done by writing any buffered output bytes to the
	...
	/**
	 * Closes the stream. This is done by flushing the stream and then closing the
	 ...
	 * set to <code>true</code> when the underlying output stream throws an
	 * <code>IOException</code> other than <code>InterruptedIOException</code>, and
	 * when the <code>setError</code> method is invoked. If an operation on the
	 * underlying output stream throws an <code>InterruptedIOException</code>, then
	 * the <code>PrintStream</code> converts the exception back into an interrupt by
	 * doing:
	 * 
	 * <pre>
	...
	/*
	 * Exception-catching which also implement the write() methods of OutputStream
	 */
	...
	/*
	 * The following methods on the text- and character-output streams always flush
	 * the stream buffers, so that writes to the underlying byte stream occur as
	 * promptly as with the original PrintStream.
	 */
	 ...
	 *                                          illegal syntax, a format specifier
	 *                                          that is incompatible with the given
	 ...
	 * @since 1.5
	 * 
	 *        public PrintStream printf(String format, Object... args) { return
	 *        format(format, args); }
	 */
	 ...
	 * {@link java.util.Locale#getDefault() Locale.getDefault()}, regardless of any
	 ...
	 * @since 1.5
	 * 
	 *        public PrintStream format(String format, Object... args) { try {
	 *        ensureOpen(); if ((formatter == null) || (formatter.locale() !=
	 *        Locale.getDefault())) formatter = new Formatter((Appendable) this);
	 *        formatter.format(Locale.getDefault(), format, args); } catch
	 *        (IOException x) { trouble = true; } return this; }
	 */
	 ...
	 *        try { ensureOpen(); if ((formatter == null) || (formatter.locale() !=
	 *        l)) formatter = new Formatter(this, l); formatter.format(l, format,
	 *        args); } catch (IOException x) { trouble = true; } return this; }
	 */
	 ...
	 * invoking then <tt>toString</tt> method of a character buffer will return a
